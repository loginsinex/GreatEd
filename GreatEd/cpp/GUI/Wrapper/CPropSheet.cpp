...
{
	CIconBits icon;
	icon.ImportIcon( hIcon );
	AddSheet( dlg, lParam, icon , pszTitle, fForceCreate);
}

void CPropSheet::AddSheet(CDialog & dlg, LPARAM lParam, CIconBits & icon, LPCTSTR pszTitle, BOOL fForceCreate)
{
	PROPSHEET_ITEM ppi;
...
INT_PTR CPropSheet::InternalShow(PROPSHEETHEADER & psh, CWindow * pParent, LPARAM lParam)
{
	...
	for(size_t i = 0; i < m_vpDialogs.size(); ++i)
	{
		...
		SIZE sz = item.icon.Size();
		if ( sz.cx > 0 && sz.cy > 0 )
		{
			psp.dwFlags |= PSP_USEHICON;
			HICON hIcon = item.icon.ExportIcon();
			psp.hIcon = hIcon;
			m_vCreatedIcons.push_back( hIcon );
		}
