...
BOOL __handle_accel_table( HWND hActiveWindow, MSG & msg )
{
	...
	auto dwExStyle = GetWindowLongPtr( hActiveWindow, GWL_EXSTYLE );
	if ( WS_EX_MDICHILD & dwExStyle ) // is there mdi child? Translate mdi child accel first
	{
		if ( TranslateMDISysAccel( GetParent( hActiveWindow ), &msg ) )
		{
			return TRUE;
		}
	}
...
BOOL __handle_global_accel( MSG & msg )
{
	...
	if ( __enter_accel_tables() )
	{
		...
		if ( mg != l_mGlobalAccelWnd.end() )
		{
			...
			if ( me != mThreadAccelTable.end() )
			{
				auto dwExStyle = GetWindowLongPtr( me->first, GWL_EXSTYLE );
				if ( WS_EX_MDICHILD & dwExStyle ) // is there mdi child? Translate mdi child accel first
				{
					fResult = TranslateMDISysAccel( GetParent( me->first ), &msg );
				}

				if ( !fResult ) fResult = TranslateAccelerator( me->first, me->second, &msg );
			}
		}
		__leave_accel_tables();
	}
...
VOID CControl::InitCtl()
{
	...
	/* just for test
	BOOL( CALLBACK* fnAllowDarkModeForWindow )( HWND, BOOL );
	BOOL( CALLBACK* fnSetPreferredAppMode )( DWORD );
	HINSTANCE huxtheme = LoadLibrary( TEXT( "uxtheme.dll" ) );
	*( (PVOID*) &fnAllowDarkModeForWindow ) = (PVOID) GetProcAddress( huxtheme, LPCSTR( 133 ) );
	*( (PVOID*) &fnSetPreferredAppMode ) = (PVOID) GetProcAddress( huxtheme, LPCSTR( 135 ) );

	if ( fnAllowDarkModeForWindow ) fnAllowDarkModeForWindow( m_hWnd, TRUE );
	if ( fnSetPreferredAppMode ) fnSetPreferredAppMode( 2 );
...
CWindow::CWindow(HINSTANCE hInstance, tstring sClass, BOOL fAddDefaultButtons)
	...
	m_fInFocus( FALSE ), m_hPropSheet( NULL ), m_fWindowModal( FALSE ), m_uWindowModalExitCode( 0 ), m_fSelfHandle( FALSE ), m_fGlobalAccel( FALSE ), m_ftWindowType( ftWindow ),
...
CWindow::CWindow( HINSTANCE hInstance, LONG_PTR idRes )
	...
	m_fInFocus( FALSE ), m_fWindowModal( FALSE ), m_uWindowModalExitCode( 0 ), m_fSelfHandle( FALSE ), m_fGlobalAccel( FALSE ), m_ftWindowType( ftWindow )
{
	__init_accel_tables();
}
...
LRESULT CWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHILDACTIVATE: OnChildActivate(); return WindowProcDefault( uMsg, wParam, lParam );
	...
	case WM_MOVE: OnMove( GET_X_LPARAM( lParam ), GET_Y_LPARAM( lParam ) ); break;
...
VOID CWindow::OnMove( SHORT x, SHORT y )
{
	return;
}
...
BOOL CWindow::IsWindowModal() const
{
...
INT CWindow::MsgBox(LPCTSTR pszText, LPCTSTR pszCaption, UINT uType) const
{
...
VOID CWindow::ShowMessage(LPCTSTR pszText) const
{
	MsgBox(pszText, NULL, MB_ICONINFORMATION);
}

VOID CWindow::ShowMessageF(LPCTSTR pszText, ...) const
{
	va_list argptr;
	CString msg;
...
VOID CWindow::ShowAlert(LPCTSTR pszText) const
{
	MsgBox(pszText, NULL, MB_ICONWARNING);
}

VOID CWindow::ShowAlertF(LPCTSTR pszText, ...) const
{
	va_list argptr;
	CString msg;
...
VOID CWindow::ShowError(LPCTSTR pszText) const
{
	MsgBox(pszText, NULL, MB_ICONERROR);	
}

VOID CWindow::ShowErrorF(LPCTSTR pszText, ...) const
{
	va_list argptr;
	CString msg;
...
BOOL CWindow::Confirm(LPCTSTR pszText, BOOL fDefaultNo) const
{
	return ( IDYES == MsgBox(pszText, NULL, MB_ICONQUESTION | MB_YESNO | ( fDefaultNo ? MB_DEFBUTTON2 : 0 )) );
}

BOOL CWindow::ConfirmF(LPCTSTR pszText, BOOL fDefaultNo, ...) const
{
	va_list argptr;
	CString msg;
