	...
	: CEngineHackDlg( hInstance, TEXT( "Coins for 1-UP" ), hack )
{
	...
	{
		CString strWorld;
		strWorld.Format( TEXT( "World #%d:" ), i + 1 );
		m_stWorld[ i ].PostInit( hInstance, 0x1000 + i, WC_STATIC, strWorld, left, top + 2 + 25 * i, 50, 10, SS_RIGHT | WS_VISIBLE );
		m_edWorld[ i ].PostInit( hInstance, 0x2000 + i, WC_EDIT, nullptr, left + 60, top + 25 * i, 40, 13, WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL | WS_BORDER );
		m_udWorld[ i ].PostInit( hInstance, 0x3000 + i, UPDOWN_CLASS, nullptr, 0, 0, 0, 0, WS_VISIBLE | UDS_ALIGNRIGHT | UDS_ARROWKEYS | UDS_AUTOBUDDY | UDS_SETBUDDYINT | UDS_WRAP );
...
BOOL CEngineHackCoins1UP::OnInit( LPARAM lParam )
{
	for ( USHORT i = 0; i < NES_WORLDS_COUNT; ++i )
	{
		m_udWorld[ i ].SetRange( 99, 0 );
		m_udWorld[ i ].Value( Hack().coins.bCoins[ i ] );
	}

	SetFocus( m_edWorld[ 0 ] );
	return FALSE;
}
...
BOOL CEngineHackCoins1UP::PSOnApply( BOOL fOkPressed )
{
	for ( USHORT i = 0; i < NES_WORLDS_COUNT; ++i )
	{
		if ( m_udWorld[ i ].IsError() )
		{
			ShowErrorF( TEXT( "Erroneous value for world #%d" ), i + 1 );
			return PSNRET_INVALID;
		}
		else
		{
			Hack().coins.bCoins[ i ] = LOBYTE( m_udWorld[ i ].Value() );
		}
	}
	...
	: CEngineHackDlg( hInstance, TEXT( "Warp zones" ), hack )
{
	for ( USHORT z = 0; z < 3; ++z )
	{
		...
		m_stZone[ z ].PostInit( hInstance, 0x1000, WC_STATIC, strFormat, 20, 22 + 25 * z, 50, 10, SS_RIGHT | WS_VISIBLE );
		pushctl( m_stZone[ z ] );

		for ( USHORT w = 0; w < 3; ++w )
		{
			m_cbWorld[ z ][ w ].PostInit( hInstance, 0x2000 + 3 * z + w, WC_COMBOBOX, nullptr, 80 + 65 * w, 20 + 25 * z, 60, 150, WS_VISIBLE | WS_TABSTOP | CBS_DROPDOWNLIST | WS_VSCROLL );
			pushctl( m_cbWorld[ z ][ w ] );
		}
	}
}
...
BOOL CEngineHackWarpZones::OnInit( LPARAM lParam )
{
	...
	for ( USHORT z = 0; z < 3; ++z )
	{
		for ( USHORT w = 0; w < 3; ++w )
		{
			const BYTE bWorld = Hack().warp.bWorlds[ z ][ w ];
			m_cbWorld[ z ][ w ].cSendMessage( CB_ADDSTRING, 0, (LPARAM)TEXT( "Not used" ) );
			m_cbWorld[ z ][ w ].cSendMessage( CB_SETCURSEL );

			for ( USHORT i = 0; i < NES_WORLDS_COUNT; ++i )
			{
				CString strFormat;
				strFormat.Format( TEXT( "World #%d" ), i + 1 );
				m_cbWorld[ z ][ w ].cSendMessage( CB_ADDSTRING, 0, (LPARAM)strFormat.GetString() );
			}

			if ( bWorld >= 1 && bWorld <= NES_WORLDS_COUNT )
			{
				m_cbWorld[ z ][ w ].cSendMessage( CB_SETCURSEL, bWorld );
			}
		}
	}

	SetFocus( m_cbWorld[ 0 ][ 0 ] );
	return FALSE;
}
...
BOOL CEngineHackWarpZones::PSOnApply( BOOL fOkPressed )
{
	for ( USHORT z = 0; z < 3; ++z )
	{
		for ( USHORT w = 0; w < 3; ++w )
		{
			BYTE bWorld = static_cast<BYTE>( m_cbWorld[ z ][ w ].cSendMessage( CB_GETCURSEL ) );
			if ( bWorld >= 1 && bWorld <= NES_WORLDS_COUNT )
			{
				Hack().warp.bWorlds[ z ][ w ] = bWorld;
			}
			else
			{
				Hack().warp.bWorlds[ z ][ w ] = 0x24;
			}
		}
	}
...
BOOL CEngineHackHardLevel::PSOnApply( BOOL fOkPressed )
{
	...
	if ( bWorld < NES_WORLDS_COUNT && !m_udLevel.IsError() )
	{
