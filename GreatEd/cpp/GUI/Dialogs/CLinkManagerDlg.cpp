...
CLinkManagerDlg::CLinkManagerDlg( HINSTANCE hInstance )
	...
	m_pKeeper( nullptr ), m_name( hInstance )
{
	pushctl( m_tv );
	pushctl( m_stInfo );
	pushctl( m_cv );
...
BOOL CLinkManagerDlg::OnInit( LPARAM lParam )
{
	for ( auto & item : m_mLinks )
	{
		...
		for ( const auto & world : wol )
		{
			...
			strFormat.Format( TEXT( "World #%d" ), bWorld + 1 );
...
VOID CLinkManagerDlg::SelectItem( HTREEITEM hItem )
{
	...
	if ( pItem )
	{
		switch ( pItem->ti )
		{
			...
			case tiLink:
				{
					CNesLevelInterface * pInterface = m_pKeeper->GetInterface( pItem->bWorld, m_bmView, pItem->link.second );
					FreeCanvasInterface();
					m_cv.SetView( pInterface );
...
VOID CLinkManagerDlg::Set_Param( CNesLevelsKeeper * pKeeper, const LEVEL_ITEM_LINK & lnk, CNesLevel * pCurrentLevel, BYTE bWorld )
{
	m_pKeeper = pKeeper;
	m_mLinks = lnk;
	m_bWorld = bWorld;
