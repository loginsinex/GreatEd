...
BOOL CSelectLevelDlg::OnInit( LPARAM lParam )
{
	...
	for ( const auto & world : m_nAreaList )
	{
		...
		if ( world.size() > 0 && world[ 0 ].size() > 0 )
		{
			...
			strWorld.Format( TEXT( "World #%d" ), bWorld + 1 );
			HTREEITEM hItem = m_tv.AddItem( TVI_ROOT, strWorld.GetString(), param.lParam, TRUE, FALSE, TRUE );
			...
			for ( const auto & level : world )
			{
				...
				if ( level.size() > 0 )
				{
					...
					HTREEITEM hLevelItem = m_tv.AddItem( hItem, strWorld.GetString(), levelParam.lParam, FALSE, FALSE, TRUE );
					...
					if ( level.size() > 1 )
					{
						...
						for ( auto pLevel : level )
						{
							...
							HTREEITEM hAreaItem = m_tv.AddItem( hLevelItem, strWorld.GetString(), areaParam.lParam );
					...
					case TVN_SELCHANGED:
						{
							...
							m_lParam.lParam = LODWORD( ptvi->itemNew.lParam );
							break;
						}
				}
				break;
