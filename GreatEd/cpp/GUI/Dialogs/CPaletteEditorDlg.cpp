...
CPaletteEditorDlg::CPaletteEditorDlg( HINSTANCE hInstance, PAL_DLG_TYPE ndlgType )
	: CDialog( hInstance, 200, ( ndlgType == pdtFull ) ? 220 : 100 , TEXT( "Palette editor" ), TRUE ), m_ndlgType( ndlgType )
{

	USHORT counter = 24;
	if ( pdtShort == m_ndlgType ) counter = 9;
			...
			case pdtShort:
				{
					switch ( k )
					{
					case 0: sLabel.Format( TEXT( "Mario palette:" ) ); break;
					case 1: sLabel.Format( TEXT( "Luigi palette:" ) ); break;
					case 2: sLabel.Format( TEXT( "Fire Mario/Luigi:" ) ); break;
					}
				}
			}
...
VOID CPaletteEditorDlg::Set_Param( const NES_PLAYER_PAL & npc )
{
	m_npc = npc;
}

VOID CPaletteEditorDlg::Get_Param( NES_PLAYER_PAL & npc )
{
	npc = m_npc;
}
...
BOOL CPaletteEditorDlg::OnInit( LPARAM lParam )
{
	...
	case pdtShort:
		{
			for ( USHORT pal = 0; pal < npcCount; ++pal )
			{
				for ( USHORT col = 1; col < 4; ++col )
				{
					m_btnColor[ c ].cSendMessage( BM_SETIMAGE, IMAGE_BITMAP, (LPARAM)g_hbmColor[ m_npc.bPal[ pal ][ col ] ] );
					m_btnMap[ 0x1000 + c ] = { pal, col };
					c++;
				}
			}
...
VOID CPaletteEditorDlg::OnButton( USHORT uId )
{
	...
	if ( itCol != m_btnMap.end() )
	{
		...
		BYTE & bCol = ( pdtFull == m_ndlgType ? m_npa.bPal[ pal.first ][ pal.second ] : m_npc.bPal[ pal.first ][ pal.second ] );
...
CPaletteDlg::CPaletteDlg( HINSTANCE hInstance )
	...
	m_btnPlayerEdit( hInstance, 0x102, TEXT( "Modify Player palette..." ), 7, -( 7 + 15 ), 100, 15 )
{
	pushctl( m_stPaletteId );
	pushctl( m_cbPaletteId );
	pushctl( m_btnEdit );
	pushctl( m_btnPlayerEdit );
}
...
VOID CPaletteDlg::OnButton( USHORT uId )
{
	if ( 0x101 == uId )
	{
		...
		if ( id >= 0 )
		{
			...
			if ( itPal != m_mPal.end() )
			{
				CPaletteEditorDlg dlg( GetInstance(), pdtFull );
	...
	{
		CPaletteEditorDlg dlg( GetInstance(), pdtShort );
		dlg.Set_Param( m_npc );
...
VOID CPaletteDlg::Set_Param( const std::map<NES_PAL_TABLE, NES_PAL_ARRAY> & mPal, const NES_PLAYER_PAL & npc )
{
	m_mPal = mPal;
	m_npc = npc;
}

VOID CPaletteDlg::Get_Param( std::map<NES_PAL_TABLE, NES_PAL_ARRAY> & mPal, NES_PLAYER_PAL & npc )
{
	mPal = m_mPal;
	npc = m_npc;
}
