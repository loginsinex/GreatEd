...
CCreateNewAreaDlg::CCreateNewAreaDlg( HINSTANCE hInstance )
	...
	m_bWorld( 0 ), m_uType( nltGround ), m_pKeeper( nullptr ), m_pLevel( nullptr ), m_fAutoLevel( FALSE )
{
	pushctl( m_rdNewArea );
...
VOID CCreateNewAreaDlg::Get_Param( NES_LEVEL_TYPE & uType, BOOL & fAutoLevel, CNesLevel ** pBaseLevel )
{
	uType = m_uType;
	*pBaseLevel = m_pLevel;
