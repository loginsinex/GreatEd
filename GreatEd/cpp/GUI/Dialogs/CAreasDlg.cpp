...
CAreasDlg::CAreasDlg( HINSTANCE hInstance, CNesLevelsKeeper * pKeeper, BOOL fHideAutoLevel )
	...
	m_fHideAutoLevel( fHideAutoLevel )
{
	pushctl( m_lv );
	pushctl( m_ed );
	pushctl( m_ud );
...
BOOL CAreasDlg::OnInit( LPARAM lParam )
{
	...
	m_lv.AddColumn( TEXT( "Areas" ) );
	m_lv.EnableGroups( TRUE );
	
	for ( const auto & t : type )
	{
		int group = LODWORD( m_lv.AddGroup( t, strGroup[ t ] ) );

		size_t cAreas = m_lvlKeeper.AreasCount( t );
		for ( size_t i = 0; i < cAreas; ++i )
		{
			CString str;
			CNesLevel * pLevel = m_lvlKeeper.EnumAreas( t, i );

			if ( ( pLevel->GetLevelStatus() & 2 ) && m_fHideAutoLevel )
			{
				continue;
			}

			str.Format( TEXT( "%s%sArea: %d" ), 
				( ( pLevel->GetLevelStatus() & 4 ) ? TEXT( "[!] " ) : TEXT( "" ) ),
				( ( pLevel->GetLevelStatus() & 2 ) ? TEXT( "* " ) : TEXT( "" ) ), 
				pLevel->AreaPtr().bAreaId + 1 );

			INT_PTR item = m_lv.AddItem( str, (LPARAM)pLevel, group );
			if ( !m_pLevel && pLevel->AreaPtr() == m_pParentLevel->AreaPtr() || m_pLevel && pLevel->AreaPtr() == m_pLevel->AreaPtr() )
			{
