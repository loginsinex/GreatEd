...
VOID CLoopSettingsDlg::OnOK()
{
	NES_LOOP_COMMAND lp;
	m_pLoop->GetLoop( lp );
	if ( m_pLoop->LoopConnected() && lp.bPage < NES_LOOP_OWNED_COMMAND_MINIMUM_PAGE ||
		 !m_pLoop->LoopConnected() && lp.bPage < NES_LOOP_COMMAND_MINIMUM_PAGE )
	{
		ShowAlertF( TEXT( "For not connected loop commands Trigger page cannot be less than %d." ), NES_LOOP_OWNED_COMMAND_MINIMUM_PAGE );
		return;
	}
...
VOID CLoopSettingsDlg::Get_Param( NES_LOOP_COMMAND & nlp )
{
	m_pLoop->GetLoop( nlp );
}
