...
/*
BOOL CNesLevel::IsEqualPointersVectorTail( const std::vector<NES_LEVEL_ENEMY_ITEM>& v, const std::vector<NES_LEVEL_ENEMY_ITEM>& tail )
{
	if ( v.size() > tail.size() )
	{
		size_t cTailSize = tail.size();
		size_t nvFrom = v.size() - cTailSize;

		for ( size_t i = 0; i < cTailSize; ++i )
		{
			const NES_LEVEL_ENEMY_ITEM it1 = v[ i + nvFrom ];
			const NES_LEVEL_ENEMY_ITEM it2 = tail[ i ];

			if ( it1.item.bData[ 1 ] != it2.item.bData[ 1 ] || it1.item.bData[ 2 ] != it2.item.bData[ 2 ] )
			{
				return FALSE;
			}
		}
...
VOID CNesLevel::DumpPointersAsObjects( std::vector<NES_LEVEL_ENEMY_ITEM>& vItem )
{
	for ( const auto & obj : m_obj.vpItem )
	{
		if ( obj->IsEnterableObject() )
		{
			...
			/*
			if ( !IsEqualPointersVectorTail( vItem, vLocalItem ) )
			{
			...
			case NES_SPECIAL_OBJECT_F:
				{
					...
					if ( !fPageSel )
					{
						...
						{
							_tprintf( TEXT( "WARNING! Enemy page object points to BEFORE page at %d\n" ), m_lvlPtr.bPtr );
						}
					}
					/*
					else
