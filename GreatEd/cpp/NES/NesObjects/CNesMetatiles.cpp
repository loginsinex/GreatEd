...
CNesMetatiles::CNesMetatiles( CNesSimpleMMC3 & file, const CNesPointers & eptr, const CNesObjectData & obj )
	: m_file( file ), m_eptr( eptr ), m_ntNCache( m_file ), m_palette( file, eptr, obj )
{

}

VOID CNesMetatiles::CacheTiles()
{
	m_ntNCache.CacheTiles();
}
...
USHORT CNesMetatiles::GetVRAMAddrTablePtr( NES_VRAM_TABLE tableId )
{
	...
	return MAKEWORD( m_file.Data<BYTE>( uVRAMAddrTableLo + tableId ), m_file.Data<BYTE>( uVRAMAddrTableHi + tableId ) );
}
...
VOID CNesMetatiles::SetPlayerPalOption( NES_PLAYER_COL_T npcOption )
{
	m_palette.SetPlayerColOption( npcOption );
}
...
VOID CNesMetatiles::DrawTile( CPaletteBitmap & bmp, INT x, INT y, BYTE tile, int uReservedSpace ) const
{
	m_ntNCache.DrawTile( bmp, tile, x, y, uReservedSpace );
}

VOID CNesMetatiles::DrawAttribute( CPaletteBitmap & bmp, INT x, INT y, BYTE attr, int uReservedSpace ) const
{
	m_ntNCache.DrawAttribute( bmp, attr, x, y, uReservedSpace );
}

// ---- nes palette ----

CNesPalette::CNesPalette( CNesSimpleMMC3 & file, const CNesPointers & eptr, const CNesObjectData & obj )
	: m_file( file ), m_eptr( eptr ), m_bgDefault( 0x0F ), m_obj( obj ), m_npcOption( npcMario )
{
...
{
	USHORT uVRAMAddrTableLo = m_eptr.Pointer( eVRAMAddrTableLo ) + static_cast<USHORT>( tableId ), uVRAMAddrTableHi = m_eptr.Pointer( eVRAMAddrTableHi ) + static_cast<USHORT>( tableId );
	return MAKEWORD( m_file.Data<BYTE>( uVRAMAddrTableLo ), m_file.Data<USHORT>( uVRAMAddrTableHi ) );
}
...
VOID CNesPalette::LoadVRAMTable( NES_VRAM_TABLE nvt )
{
	...
	if ( uVRAMTablePtr < 0x8000 ) return;	// pointer may points to RAM
...
VOID CNesPalette::GetPalette( NES_PAL_ARRAY & pal, NES_LEVEL_TYPE nlt, NES_AREA_STYLE nas, NES_BG_COLOR_CTL bgModifier, BOOL fApplyDefaultBg )
{
	...
	if ( fApplyDefaultBg )
	{
		const auto & npc = m_obj.PlayerPal();

		for ( int i = 0; i < 4; ++i )
		{
			pal.bPal[ 1 ][ i ] = npc.bPal[ m_npcOption ][ i ];
		}
...
VOID CNesPalette::SetPlayerColOption( NES_PLAYER_COL_T npcOption )
{
	if ( npcOption < npcCount )
	{
		m_npcOption = npcOption;
	}
}
