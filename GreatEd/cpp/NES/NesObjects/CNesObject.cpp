...
CNesObject::CNesObject( CNesLevel & nLevel, BOOL fSprite )
	: m_nLevel( nLevel ), m_file( nLevel.ROM() ), m_fSprite( fSprite ), m_uFixedY( -1 ), m_uOverrideY( 0xFF ), m_fBehind( FALSE )
{
...
VOID CNesObject::DrawObject( CNesLevelView & view, BYTE bWorldId )
{
	...
	DrawSprite( view, ptObject.x, ptObject.y );
}

VOID CNesObject::DrawSprite( CNesLevelView & view, int x, int y )
{
	DrawSpriteExtra( view, x, y, FALSE );
...
VOID CNesObject::DrawExtra( CNesLevelView & view, BYTE bWorldId, int x, int y, BOOL fBeforeObject )
{
	// do nothing. Only for derived classes
}

VOID CNesObject::DrawSpriteExtra( CNesLevelView & view, int x, int y, BOOL fAfterObject )
{
	// do nothing. Only for derived classes
}
