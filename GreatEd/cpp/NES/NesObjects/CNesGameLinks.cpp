...
CNesLink::CNesLink( BYTE uSourceWorld, CNesLevel * pSourceLevel )
	: m_pSourceLevel( pSourceLevel ), m_bWorld( uSourceWorld ),
	m_pDestLevel( pSourceLevel ), m_uDestPage( 0 ), m_ref( 0 )
{
...
VOID CNesLink::AddRef()
{
	m_ref++;
}

BOOL CNesLink::Release()
{
	if ( m_ref > 0 )
	{
		m_ref--;
...
CNesLink * CNesGameLinks::AddLink( BYTE bWorld, CNesLevel * pLevel )
{
	...
	CNesLink * pLink = new CNesLink( bWorld, pLevel );
