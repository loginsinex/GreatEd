...
VOID CNesCanvas::SetView( CNesLevelInterface * pInterface )
{
	LockDraw();
	DelView();
	RenderView();	// fill rect with black
...
VOID CNesCanvas::RenderView()
{
	if ( m_pInterface )
	{
		...
		else
		{
			...
			m_bm.FillRect( 0, 0, m_szControl.cx, m_szControl.cy, m_pInterface->GetBgColor() );
			m_pInterface->RenderLevel( m_bm );
		}
	}
	else
	{
		m_bm.FillRect( 0, 0, m_bm.Width(), m_bm.Height(), RGB( 0, 0, 0 ) );
	}
}
