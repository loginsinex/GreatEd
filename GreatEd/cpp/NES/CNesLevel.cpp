...
CNesLevel * CNesLevel::Copy()
{
	...
	CNesLevel * pCopy = new CNesLevel( m_file, m_lvlPtr, m_eptr, m_mTiles, m_objData, m_loop, m_gLinks, m_title, m_player );
...
NES_AREA_PTR CNesLevel::AreaPtr() const
{
	return m_lvlPtr;
}

const CNesLevelHeader & CNesLevel::Header() const
{
	return m_header;
}
...
NES_LEVEL_TYPE CNesLevel::AreaType() const
{
	return m_lvlPtr.bType;
}

SIZE CNesLevel::LevelSize() const
{
	SIZE sz = { m_uPages * 16, 16 };
	return sz;
}

BYTE CNesLevel::PagesCount() const
{
...
VOID CNesLevel::SetupGroundItems( std::vector<NES_DRAW_QUEUE_ITEM>& vdqi, BOOL fBehind )
{
...
VOID CNesLevel::SetupBehindFlag( const std::vector<NES_DRAW_QUEUE_ITEM>& vdqi, BOOL fBehind )
{
	for ( const auto & dqi : vdqi )
	{
		if ( dqi.pGround )
...
int CNesLevel::AssignLink( CNesLink * pLink, int uBeginPoint )
{
	...
	for ( auto & obj : m_obj.vpItem )
	{
		if ( obj->IsEntranceObject() )
		{
			...
			if ( rcObject.left + 12 >= uBeginPoint )
			{
				obj->SetLink( pLink );
				uCounter++;
			}
...
BOOL CNesLevel::AddPage( UINT uBeforePage )
{
	...
	for ( auto it = m_obj.vpItem.begin() + 1; it < m_obj.vpItem.end(); ++it )
	{
		...
		if ( 0x4B == pItem->Item().item.itemId && NES_SPECIAL_OBJECT_D == pItem->Item().item.Y )
		{
			...
			if ( pLoop->TriggerPage() >= uBeforePage )
			{
				pLoop->TriggerPage( pLoop->TriggerPage() + 1 );
			}
		}
	}
...
BOOL CNesLevel::DumpLoops( std::vector<NES_LOOP_COMMAND>& vnlp )
{
	vnlp.clear();
	for ( auto it = m_obj.vpItem.begin() + 1; it != m_obj.vpItem.end(); ++it )
	{
...
BOOL CNesLevel::ReadAreaLinks( std::map<BYTE, std::pair<UINT, CNesLevel*>>& mLinks )
{
	if ( m_liGround.IsEnterableObject() )
	{
		m_liGround.ReadLinks( mLinks );
...
VOID CNesLevel::Meta( const tstring & strName, DWORD dwValue )
{
	m_meta[ strName ] = dwValue;
}

DWORD CNesLevel::Meta( const tstring & strName ) const
{
	auto it = m_meta.find( strName );
	if ( it == m_meta.end() ) return 0;
	return it->second;
}