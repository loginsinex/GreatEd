...
CNesLevelInterface::CNesLevelInterface( CNesMetatiles & mTiles, CNesGameEngineHack & hack, CPaletteBitmap & bm, CNesLevel & level, BYTE uWorldId, BOOL fRenderTitle )
	: m_view( bm, mTiles, fRenderTitle ), m_level( level ), m_uWorldId( uWorldId ), m_coinColors( hack.Read().rotateCols ), m_bCoinColor( 0 )
{
	m_level.InitObjects();
...
COLORREF CNesLevelInterface::GetBgColor()
{
	return XCHGCOLORREF( m_level.GetBgColor() );
}
...
BOOL CNesLevelInterface::SetLevelTitleVisible( BOOL fVisible )
{
	return m_view.SetTitleVisible( fVisible );
}