...
size_t CNesLevel::SerializeLevelData( std::vector<BYTE>& vLevelData )
{
	...
	*/
	...
	for ( const auto & pgLoop : mLoop )
	{
		...
		dummyItem.item.itemId = 0x4F;	// 0x4d...0x4f - prohibited objects for Y=0x0D, so use it for dummy
...
{
	if ( !( NES_SPECIAL_OBJECT_D == item.item.Y && item.item.itemId >= 0x4D && item.item.itemId <= 0x4F ) )	// prohibited (dummy?) object found
	{
	...
	while ( areaIt < areaEnd )
	{
		...
		if ( item.pt.y == NES_SPECIAL_OBJECT_D && !( item.item.itemId & 0x40 ) )
		{
			if ( !fPageSel )
			{
				...
				{
					_tprintf( TEXT( "WARNING: Page objects points to BEFORE page at %d!\n" ), m_lvlPtr.bPtr );
				}
			}
			/*
			else
		...
		else
		{
			...
			if ( item.pt.y == NES_SPECIAL_OBJECT_E || ( NES_SPECIAL_OBJECT_D == item.pt.y && item.item.itemId == 0x4C ) )
			{
...
CNesLevelItem * CNesLevel::FetchSingleItem( const NES_LEVEL_ITEM & item )
{
	switch ( item.item.itemId )
	{
		...
		case 0x07:
		case 0x08:
		case 0x0A: return new CNesQBlock( *this, item );
		case 0x09: return new CNesWaterPipe( *this, item );
		case 0x0B: return new CNesJumpspring( *this, item );
		case 0x0C: return new CNesIntroPipe( *this, item ); 
		case 0x0D: return new CNesFlagpoleObject( *this, item );
		case 0x0E: return new CNesAxeObject( *this, item );
		case 0x0F: return new CNesChainObject( *this, item );
	}
	return nullptr;
}
...
CNesLevelItem * CNesLevel::FetchMasvItemF( const NES_LEVEL_ITEM & item )
{
	...
	switch ( itemId )
	{
		...
		case 6: break;	// invicible Q-block[1]
		case 7: break;	// invicible Q-block[2]
	}
	return nullptr;
}