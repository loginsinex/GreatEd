...
typedef enum _tagNES_VRAM_TABLE : BYTE
{
	...
	nvtMarioThanksMessage,
	nvtLuigiThanksMessage,
	nvtMushroomRetainerSaved,
	nvtPrincessSaved1,
	nvtPrincessSaved2,
	nvtWorldSelectMessage1,
	nvtWorldSelectMessage2,
	nvtpVRAMTablesCount,
	nvtpSpecialWater,
	nvtpSpecialGround,
...
class CNesPalette
{
	...
	const CNesObjectData &										m_obj;
	BYTE														m_bgDefault;
	NES_PLAYER_COL_T											m_npcOption;
...
public:
	CNesPalette( CNesSimpleMMC3 & file, const CNesPointers & eptr, const CNesObjectData & obj );
	...
	VOID					SetPlayerColOption( NES_PLAYER_COL_T npcOption );
};
...
public:
	CNesMetatiles( CNesSimpleMMC3 & file, const CNesPointers & eptr, const CNesObjectData & obj );
	VOID									Load();
	VOID									Dump();
	...
	VOID									DrawTile( CPaletteBitmap & bmp, INT x, INT y, BYTE tile, int uReservedSpace ) const;
	VOID									DrawAttribute( CPaletteBitmap & bmp, INT x, INT y, BYTE attr, int uReservedSpace ) const;
};
