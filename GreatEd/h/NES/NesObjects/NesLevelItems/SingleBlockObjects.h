...
#include "h\NES\NesObjects\CNesLevelItem.h"
...
public:
	...
	BOOL				IsExternalSet();
	VOID				SetLoop( const NES_LOOP_COMMAND & lp );
	VOID				GetLoop( NES_LOOP_COMMAND & lp );
	BOOL				LoopConnected();
	VOID				LoopConnected( BOOL fConnected );
	BYTE				AllowedHeight();
	VOID				AllowedHeight( BYTE Y );
	BYTE				TriggerPage();
	VOID				TriggerPage( BYTE bPage );
	VOID				RewindOffset( BYTE bRewindOffset );
};